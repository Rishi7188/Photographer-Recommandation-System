...
def find_nearest_photographers():
    ...
    # Calculate distance between user and each photographer
    for p in all_photographers:
        distance = haversine(user_latitude, user_longitude, float(p.latitude), float(p.longitude))
        p.distance = distance  # Add distance to photographer object

        # Retrieve average rating for the photographer
        avg_rating = db.session.query(func.avg(feedback.rating)).filter(feedback.photographer_id == p.photographer_id).scalar()
        p.rating = avg_rating if avg_rating else 0  # Set rating to 0 if there are no feedback entries for the photographer

    # Sort photographers by distance (ascending order) and rating (descending order)
    nearest_photographers = sorted(all_photographers, key=lambda x: (x.distance, x.rating), reverse=True)

    # Format the profile_picture path and filter out photographers without a profile picture
    for p in nearest_photographers:
        if p.profile_picture:
            p.profile_picture = p.profile_picture.replace('static\\', '').replace('\\', '/')
        else:
            p.profile_picture = '/default_profile_picture.jpg'  # Assuming there's a default profile picture
